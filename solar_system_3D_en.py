...
class Our_Solar_System(Entity):
    def __init__(self, **kwargs):
        ...
        self.moon_orbits = Text(parent=self.parent, text="Moon Rotations / Orbits (Earth):", x=-0.88, y=-0.135, scale=(1, 1), origin=(-.5, .5), color=color.cyan, text_size=1.0, eternal=True)
    ...
    def read_config(self):
        try:
            ...
            for key, value in data.items():
                if key=="0":self.Language = value
                elif key=="1":
                    self.start_time.text = value
                    self.Start_Time = Time(self.start_time.text)
                elif key=="2":self.latitude.text = value
                elif key=="3":self.longitude.text = value
                elif key=="4":self.altitude.text = value
                elif key=="5":self.duration.text = value
                elif key=="6":self.increment.text = value
                elif key=="7":self.delay.text = value
                else: break
        except Exception as e:
            pass
    def write_config(self):
        ...
        try:
            ...
            temp_dict[0]=self.Language
            temp_dict[1]=self.start_time.text
            temp_dict[2]=self.latitude.text
            temp_dict[3]=self.longitude.text
            temp_dict[4]=self.altitude.text
            temp_dict[5]=self.duration.text
            temp_dict[6]=self.increment.text
            temp_dict[7]=self.delay.text
    ...
    def get_moonrise_moonset(self,utc_datetime):
        try:
            ...
            if len(self.Moon_RiseSet)>0 and date_only==self.last_moonrise:
                ...
                if self.Language=="English":title="Local Moonrise / Moonset:"
                else:title="Salida / Puesta de la Luna local:"
    ...
    def get_sunrise_sunset(self,utc_datetime):
        try:
            ...
            if len(self.Sun_RiseSet)>0 and date_only==self.last_sunrise:
                ...
                if self.Language=="English":title="Local Sunrise / Sunset:"
                else:title="Amanecer / Atardecer local:"
                self.last_sunrise=date_only
                location = LocationInfo(name="Earth Location", region="Earth", timezone=self.Time_Zone, 
                                        latitude=float(self.latitude.text), longitude=float(self.longitude.text))
    ...
    def finalize_data(self):# Exaggerate Moons Orbit To Accomidate Size Of Earth. Add My Location On Earth Object
        ...
        for e in range(0, len(self.Local_Time)):
            try:
                ...
                self.Planets_Data[3][e]=self.Planets_Data[3][e-1]
                pass    
        count=0
        for e in range(0, len(self.Local_Time)):
        ...
        if not application.paused and self.Data_Ready:
            if self.delay_executed:
                ...
                if self.time_now.enabled:# If Data Enabled
                    ...
                    self.moon_orbits.text=(f'Moon Rotations / Orbits (Earth): {str(round(1 / 365.256 * (self.Increment_Days * self.dt),6))} / {str(round((self.Increment_Days * self.dt) / 27.3217,8))}')
